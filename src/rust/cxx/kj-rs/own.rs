...
/// `impl KjOwn<T> {}`. The primitive types a bridge accepts in a `KjOwn` are implemented here.
///
/// # Safety
...
/// Implements [`OwnTarget`] for the primitive types a bridge accepts in a `KjOwn`, over the drop
/// functions defined in `own.c++`.
macro_rules! impl_own_target_for_primitive {
    ($($ty:ident => $link:literal),* $(,)?) => {$(
        // Safety: `own.c++` defines the linked function as `kj::Own<$ty>::~Own()`.
        unsafe impl OwnTarget for $ty {
            unsafe fn __drop(own: *mut KjOwn<Self>) {
                unsafe extern "C" {
                    #[link_name = $link]
                    fn __drop(own: *mut KjOwn<$ty>);
                }
...
impl_own_target_for_primitive! {
    bool => "cxxbridge$kjrs$own$bool$drop",
    u8 => "cxxbridge$kjrs$own$u8$drop",
    u16 => "cxxbridge$kjrs$own$u16$drop",
    u32 => "cxxbridge$kjrs$own$u32$drop",
    u64 => "cxxbridge$kjrs$own$u64$drop",
    usize => "cxxbridge$kjrs$own$usize$drop",
    i8 => "cxxbridge$kjrs$own$i8$drop",
    i16 => "cxxbridge$kjrs$own$i16$drop",
    i32 => "cxxbridge$kjrs$own$i32$drop",
    i64 => "cxxbridge$kjrs$own$i64$drop",
    isize => "cxxbridge$kjrs$own$isize$drop",
    f32 => "cxxbridge$kjrs$own$f32$drop",
    f64 => "cxxbridge$kjrs$own$f64$drop",
}
