...
| 17 | transformer object after the stream closes (stream and controller still held) | retained | collectable (spec ClearAlgorithms; Node agrees); after abort, cancel, terminate() and error() both collect | `transformerCollectedAfterFinish` |
...
| `gc.js` | write→read handoff survives gc() with the stream dropped (--expose-gc); the transformer is collectable once the stream is closed, aborted, cancelled, terminated or errored, with the stream held (#17 for close) |
